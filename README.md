...
- When enqueueing a job, you can pass `callbackUrl` in the POST body. The worker will create a webhook delivery job which is retried with exponential backoff on failure (default 5 attempts).
- Webhook attempt counters and status keys in Redis:
	- `scan:webhook:attempts:<jobId>` — integer incremented each delivery attempt
	- `scan:webhook:lastError:<jobId>` — last error text (if any)
	- `scan:webhook:lastSuccess:<jobId>` — timestamp (ms) of last successful delivery
